...
let InputAddModol = AddModal.querySelector("input");
var ToDoItems ="hello"
let BtnEdit = document.querySelector("#myEditBtn");
...
function SetTime(){
    ...
    let time = document.querySelector(".Present");
    let DayTime = time.querySelector(".day-time");
    let MonthTime = time.querySelector(".month-time");
    let YearTime = time.querySelector(".year-time");
    Month = MnumberTostring(Month)
    DayTime.textContent = Day;
    MonthTime.textContent = Month;
    YearTime.textContent = Year;
}
function ImportItem(title){
...
function DeleteItem(){
    ...
    DeleteBtns.forEach((DeleteIcon)=>{
        DeleteIcon.addEventListener('click', function(){
            ...
            let SelectedItem = document.querySelectorAll(`div[data-index="${DataIndex}"]`)[0];
